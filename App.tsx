      ...
      <div className={`min-h-screen ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'} flex items-center justify-center p-6`}>
        <div className="max-w-lg w-full bg-white dark:bg-slate-800 rounded-[3rem] shadow-2xl p-12 text-center border-2 border-slate-100 dark:border-slate-700">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-10 leading-relaxed font-bold">
            Your investment preferences have been synced with our system. A representative from So. Florida Home Buyers LLC will contact you soon.
          </p>
          <button 
  ...
  return (
    <div className={`${isDarkMode ? 'dark' : ''}`}>
      <div className={`min-h-screen ${isDarkMode ? 'bg-slate-900' : 'bg-[#FBFBFF]'} transition-colors duration-500 pb-20`}>
        {/* High-End Navigation */}
        <nav className={`sticky top-0 z-50 w-full px-8 py-6 border-b ${isDarkMode ? 'bg-slate-900/80 border-slate-800' : 'bg-white/90 border-slate-100'} backdrop-blur-xl`}>
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className={`text-xl md:text-2xl font-black tracking-tighter leading-none ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>SO. FLORIDA HOME BUYERS LLC</h1>
                <p className={`text-[10px] uppercase font-black tracking-[0.4em] mt-1 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>Property Preferences Portal</p>
              </div>
            </div>
            <button 
              onClick={toggleDarkMode}
              className={`p-2.5 rounded-xl border-2 transition-all ${isDarkMode ? 'border-slate-700 bg-slate-800 text-slate-200' : 'border-slate-100 bg-white text-slate-600 hover:border-blue-200 shadow-sm'}`}
            >
        ...
        <main className="max-w-5xl mx-auto px-6 py-16">
          <form onSubmit={handleSubmit} className="space-y-20">
            ...
            <section className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
              <div className={`rounded-[3rem] p-10 md:p-12 shadow-2xl ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-50'} border-2 relative overflow-hidden`}>
                ...
                <div className="relative z-10 space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                      "Choose property type interested in buying",
                      "Choose special preferences for that property",
                      "Repeat steps for each property type"
                    ].map((text, i) => (
                      <div key={i} className={`p-6 rounded-[2rem] border-2 transition-all ${isDarkMode ? 'bg-slate-900/50 border-slate-700' : 'bg-blue-50/30 border-blue-50'} flex gap-4 items-center`}>
                        <span className="text-3xl font-black text-blue-600/20">{i + 1}</span>
                        <p className={`text-sm font-bold leading-tight ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>{text}</p>
                      </div>
                    ))}
                  </div>

                  <div className={`p-6 rounded-3xl border-2 flex gap-5 items-start ${isDarkMode ? 'bg-amber-900/10 border-amber-800/30' : 'bg-amber-50/50 border-amber-100'}`}>
                    <AlertCircle className="text-amber-500 w-6 h-6 shrink-0 mt-0.5" />
                    <p className={`text-sm leading-relaxed ${isDarkMode ? 'text-amber-200' : 'text-amber-900/80'}`}>
                      <span className="font-black uppercase tracking-wider mr-2 underline">Important Note:</span> If you select <span className="font-black text-blue-600 dark:text-blue-400">"ONLY"</span> under Special Preference, you'll see details <span className="italic underline">only</span> for properties that match that specific preference.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-10">
              <div className="flex items-center gap-4 px-2">
                ...
                <h3 className={`text-2xl font-black tracking-tight uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Contact Profile</h3>
              </div>
              <div className={`grid grid-cols-1 md:grid-cols-2 gap-8 p-10 md:p-14 rounded-[3.5rem] shadow-xl ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} border-2`}>
                {[
                  { id: 'name', label: 'Name*', icon: User, placeholder: 'e.g. John Wick' },
                  { id: 'company', label: 'Company*', icon: Briefcase, placeholder: 'e.g. So. Florida Home Buyers LLC' },
                  { id: 'email', label: 'Email*', icon: Mail, placeholder: 'investor@example.com' },
                  { id: 'textNumber', label: 'Text Number*', icon: Smartphone, placeholder: 'Digits only (e.g. 3055550123)' },
                  { id: 'phoneCall', label: 'Phone Call Number*', icon: PhoneCall, placeholder: 'Digits only (e.g. 3055559999)' }
                ].map((field) => (
                  <div key={field.id} className={`space-y-3 ${field.id === 'phoneCall' ? 'md:col-span-2' : ''}`}>
                    <label className={`text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                      <field.icon size={12} className="text-blue-600" /> {field.label}
                    </label>
                    <input 
                      required
                      type={field.id === 'email' ? 'email' : 'text'}
                      value={(formData.contact as any)[field.id]}
                      onChange={(e) => {
                        let val = e.target.value;
                        if (field.id === 'textNumber' || field.id === 'phoneCall') {
                          val = val.replace(/\D/g, '');
                        }
                        updateContact(field.id as keyof ContactInfo, val);
                      }}
                      ...
                      <div className="p-10 md:p-14">
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-12">
                          <div className="flex items-center gap-6">
                            ...
                            <div>
                              <h4 className={`text-3xl font-black tracking-tight uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{config.title}</h4>
                              <p className={`text-[10px] font-black uppercase tracking-[0.3em] ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Preference Settings</p>
                            </div>
                          </div>
                          
                          <label className={`flex items-center gap-6 cursor-pointer select-none px-8 py-6 rounded-[2rem] border-2 transition-all group ${state.enabled ? 'bg-blue-600 border-blue-700 text-white' : (isDarkMode ? 'bg-slate-900/50 border-slate-700 text-slate-500' : 'bg-slate-50 border-slate-100 text-slate-600 hover:bg-white hover:border-blue-100')}`}>
                            <div className="relative flex items-center">
                              <input 
                                type="checkbox"
                                checked={state.enabled}
                                ...
                                className="sr-only peer"
                              />
                              <div className={`w-14 h-7 rounded-full transition-all duration-300 ${state.enabled ? 'bg-blue-400' : 'bg-slate-300 dark:bg-slate-700'} peer-focus:outline-none relative flex items-center px-[2px] after:content-[''] after:absolute after:top-1/2 after:-translate-y-1/2 after:left-[4px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-7`}></div>
                            </div>
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="relative">
                                  <select 
                                    value={state.type}
                                    onChange={(e) => updatePropertyField(key, 'type', e.target.value)}
                                    className={`w-full px-8 py-6 rounded-2xl border-2 appearance-none transition-all outline-none focus:ring-4 ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white focus:ring-blue-500/20' : 'bg-white border-slate-100 text-slate-900 focus:ring-blue-600/10 focus:border-blue-200'}`}
                                  >
                                    <option value="">-- Select Specific Type --</option>
                                    {config.options.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                  </select>
            ...
            <section className="space-y-10">
              <div className="flex items-center gap-4 px-2">
                ...
                <h3 className={`text-2xl font-black tracking-tight uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Geographic Area</h3>
              </div>
              <div className={`grid grid-cols-1 md:grid-cols-2 gap-10 p-12 rounded-[3.5rem] shadow-xl ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} border-2`}>
                <div className="space-y-4">
                  <label className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-3 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                    County of Interest*
                  </label>
                  <div className="relative">
                    <select 
                      ...
                      className={`w-full px-8 py-6 rounded-2xl border-2 appearance-none transition-all outline-none focus:ring-4 ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white focus:ring-blue-500/20' : 'bg-slate-50 border-slate-100 text-slate-900 focus:ring-blue-600/10 focus:border-blue-200 focus:bg-white'}`}
                    >
                ...
                <div className={`space-y-4 transition-all duration-500 ${!formData.location.county ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
                  <label className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-3 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                    City of Interest*
                  </label>
                  <div className="relative">
                    ...
                    >
                      ...
                      {(CITIES_BY_COUNTY[formData.location.county] || CITIES_BY_COUNTY['default']).map(city => (
            ...
            <section className="pt-16">
              <div className={`p-14 rounded-[4rem] text-center border-4 transition-all duration-700 ${isFormValid ? 'bg-blue-600/5 border-blue-600 shadow-[0_0_80px_rgba(37,99,235,0.15)]' : 'bg-slate-50 border-slate-100 dark:bg-slate-800/30 dark:border-slate-800'}`}>
                    ...
                    {!isFormValid ? (
                      <div className="flex flex-col items-center gap-3 px-8 py-5 rounded-[2rem] bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 transition-all">
                        <div className="flex items-center gap-3">
                          ...
                          <p className={`text-[11px] font-black uppercase tracking-[0.2em] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                            Form status: INCOMPLETE
                          </p>
                        </div>
                        <p className={`text-[10px] font-bold ${isDarkMode ? 'text-slate-500' : 'text-slate-400'} italic max-w-sm leading-relaxed`}>
        ...
        <footer className={`py-24 mt-24 border-t ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-500' : 'bg-white border-slate-100 text-slate-400'}`}>
          <div className="max-w-6xl mx-auto px-8 text-center">
            <div className="flex items-center justify-center gap-4 mb-10">
              <Building2 size={32} className="text-blue-600" />
              <span className={`text-2xl md:text-3xl font-black tracking-tighter uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>SO. FLORIDA HOME BUYERS LLC</span>
            </div>
            <p className="text-sm max-w-2xl mx-auto leading-relaxed font-bold tracking-tight px-4 mb-16">
              Connecting premium capital with high-potential real estate opportunities across South Florida through precise local expertise and strategic matching.
            </p>
            <div className="flex items-center justify-center gap-10">
              <div className="h-px w-20 bg-gradient-to-r from-transparent to-slate-200 dark:to-slate-800"></div>
              <p className="text-[10px] font-black uppercase tracking-[0.6em] text-blue-600">SO. FLORIDA HOME BUYERS LLC 2025</p>
